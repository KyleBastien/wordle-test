...
const matchesFilters = (wordList: string[], filters: Filter[]): string[] => {
...
const createTreeForWord = (
  ...
  if (depth > 4) {
    ...
    colors.forEach((color) => {
      if (tree[color] == null && tree.list.length > 0) {
...
const fillInTreeForWord = (word: string, originalList: string[]): Tree => {
  const depth = 0
  const composedTree: Tree = { list: originalList, p: 1 }
...
const calculatePForTree = (
  ...
  colors.forEach((color) => {
    if (tree[color] != null && (tree[color]?.list.length ?? 0) > 0) {
      if (depth === 4) {
...
const calculateWordScore = (tree: Tree): number => {
...
function replaceAt (word: string, index: number, replaceWith: string = '-'): string {
...
function isThisTheWord (
  ...
  // yellow pass
  const timeWeSetLetterToYellow = new Map<string, number>()
  for (let i = 0; i < input.length; i++) {
    const letter = input[i]
...
// play the game
export const playGame = (word?: string): void => {
  ...
  if (word === null || word === undefined) {
    ...
    while (guess !== goalWord && numberOfGuesses < 7) {
      ...
      pastGuesses.push(guess.slice(0))
      numberOfGuesses++
      if (goalWord !== undefined || goalWord !== null) {
        guessResult.push(
          ...isThisTheWord(result.word, goalWord, numberOfGuesses)
        )
      }
  ...
  const counts = attemptCountList.reduce<number[]>(
    (acc, attempts) => {
      // TODO Need to fix this eslint error
